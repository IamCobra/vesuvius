...
export default function AdminDashboard() {
  ...
  const session = { role: "admin", name: "Test Admin" };
  const loadingSession = false;
  const [stats, setStats] = useState<any>(null);
  const [menu, setMenu] = useState<any[]>([]);
  const [ordersSummary, setOrdersSummary] = useState({ queued: 0, in_progress: 0, ready: 0, complications: 0 });
...
*/
  ...
  // Search filtering on client for snappy UX
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="bg-white p-8 rounded shadow text-center">
          ...
          <p className="mb-6">Du skal være administrator for at se denne side.</p>
          <Link href="/login" className="px-4 py-2 bg-orange-600 text-white rounded">Log ind</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm text-gray-500">Velkommen, {session.name}</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Link href="/logout" className="px-3 py-1 bg-red-600 text-white rounded text-sm">Log ud</Link>
        </div>
      </header>
      ...
      <div className="max-w-7xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <aside className="col-span-1 bg-white rounded-lg p-4 shadow">
          <nav className="space-y-2">
            <Link href="/admin" className="block py-2 px-3 rounded hover:bg-orange-50 font-semibold">Oversigt</Link>
            <a href="#menu-editor" className="block py-2 px-3 rounded hover:bg-orange-50">Menu Editor</a>
            <a href="#orders" className="block py-2 px-3 rounded hover:bg-orange-50">Ordreoverblik</a>
            <a href="#reports" className="block py-2 px-3 rounded hover:bg-orange-50">Statistik</a>
          </nav>
        </aside>
        ...
        <main className="col-span-1 lg:col-span-3 space-y-6">
          ...
          <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card title="Bordudnyttelse" value={stats?.tableUtilization ? `${Math.round(stats.tableUtilization * 100)}%` : "–"} subtitle="Booked vs tilgængelig" />
            <Card title="Top ret" value={stats?.topDish?.name || "–"} subtitle={`Solgt: ${stats?.topDish?.count || 0}`} />
          </section>

          {/* Trend chart + orders summary */}
          <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-white rounded-lg p-4 shadow" aria-labelledby="trend-title">
              <h3 id="trend-title" className="text-lg font-semibold mb-3">Omsætning — 30 dage</h3>
            ...
            <div className="bg-white rounded-lg p-4 shadow">
              ...
              <div className="mt-4">
                <Link href="/kitchen" className="block text-center px-3 py-2 bg-orange-600 text-white rounded">Gå til køkkenskærm</Link>
              </div>
            </div>
          </section>
          ...
          <section id="menu-editor" className="bg-white rounded-lg p-4 shadow">
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredMenu.map((m) => (
                <article key={m.id} className="border rounded p-3 flex flex-col">
              ...
              {filteredMenu.length === 0 && (
                <div className="col-span-full text-center text-gray-500 py-6">Ingen menupunkter fundet</div>
              )}
            </div>
          </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-3 border rounded">
                ...
                <ol className="mt-2 text-sm list-decimal list-inside">
                  {(stats?.topDishes || []).map((d: any, idx: number) => (
              ...
              <div className="p-3 border rounded">
                ...
                <p className="mt-2 text-sm">Procentdel reserverede borde: <strong>{stats?.tableUtilization ? Math.round(stats.tableUtilization * 100) + '%' : '–'}</strong></p>
                <p className="mt-2 text-xs text-gray-500">Tidsbegrænsninger på reservationer håndteres i reservations-mikrotjenesten.</p>
              </div>

              <div className="p-3 border rounded">
                <h4 className="font-semibold">Fejlhåndteringstest</h4>
                <p className="mt-2 text-sm">Kør simuleret netværksfejl og verificer DB integritet. API tilbyder en test-endpoint: <code>/api/admin/test/failure</code></p>
              </div>
            </div>
          </section>
          ...
          <section id="settings" className="bg-white rounded-lg p-4 shadow">
            <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-3 border rounded">
                <label className="block text-sm font-medium">Session timeout (min)</label>
                <input defaultValue={30} type="number" min={5} max={240} className="mt-2 px-3 py-2 border rounded w-24" />
                <p className="text-xs text-gray-500 mt-2">Standard: 30 minutter inaktivitet.</p>
              </div>

              <div className="p-3 border rounded">
                <label className="block text-sm font-medium">Password policy</label>
                <p className="text-xs text-gray-500 mt-2">Minimum 8 tegn, mindst et tal og et stort bogstav.</p>
              </div>
            </div>
          </section>
        </main>
...
function AddMenuItemButton({ onAdd }: { onAdd?: () => void }) {
  ...
  return (
    <div>
      <button onClick={() => setOpen(true)} className="px-3 py-2 bg-green-600 text-white rounded">Tilføj ret</button>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-lg">
        ...
        <div className="grid grid-cols-1 gap-3">
          <label className="text-sm">Navn <input value={name} onChange={(e) => setName(e.target.value)} className="w-full border px-2 py-1 rounded mt-1" /></label>
          <label className="text-sm">Pris (kr) <input value={price} onChange={(e) => setPrice(Number(e.target.value))} type="number" className="w-full border px-2 py-1 rounded mt-1" /></label>
          <label className="text-sm">Kategori <input value={category} onChange={(e) => setCategory(e.target.value)} className="w-full border px-2 py-1 rounded mt-1" /></label>
          <label className="text-sm">Beskrivelse <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="w-full border px-2 py-1 rounded mt-1" /></label>

          <div className="flex justify-end gap-2 mt-3">
            <button onClick={onClose} className="px-3 py-2 border rounded">Annuller</button>
            <button onClick={submit} className="px-3 py-2 bg-orange-600 text-white rounded">Gem</button>
          </div>
        </div>
      </div>
    </div>
